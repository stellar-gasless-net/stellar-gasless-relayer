...
* **Solves a real problem gasless services actually have**: anyone can drain a sponsorship budget with an unlimited number of fresh addresses, since sponsoring is otherwise free to request. `maxSponsoredTxPerUserPerDay` is enforced per Stellar source address, not per real-world identity, so the unverified cap is not sybil-resistant on its own. `maxSponsoredTxPerVerifiedUserPerDay` (optional, per-key, in `SPONSOR_POLICIES_JSON`) gives addresses holding a real verified zkident credential a higher — or unlimited — daily cap than `maxSponsoredTxPerUserPerDay`. Integrators needing identity-based abuse resistance should use the verified-user tier. Neither cap changes at all for a key that doesn't set the new field — fully backward compatible, same principle every other policy field here already follows.
